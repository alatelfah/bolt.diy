  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__heroThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__heroArtifact__', '__heroThought__', '__heroQuickAction', '__heroSelectedElement__'],

      // ['className', '__heroThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__heroArtifact__', '__heroThought__', '__heroQuickAction'],
    ],
  },
  strip: [],
};
